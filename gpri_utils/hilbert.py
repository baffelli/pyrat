...
def hilbert(arr):
    ...
    for idx_az in range(arr.shapep[1]):
        if idx_az % 100 == 0:
            print("interp azimtuh:" + str(idx_az))
        arr_h[idx_az] = _sig.hilbert(arr[:, idx_az])
...
def main():
    ...
    with open(args.output, 'wb') as of, open(args.input, 'rb') as inf:
        chan = _np.fromfile(inf, dt=data_type)
        chan_nsamp = len(chan) / args.width
        chan = chan.reshape([width, chan_nsamp])
        chan_h = hilbert(chan)
        chan_h.T.astype(data_type).tofile(of)
