...
subparser_apply = subparsers.add_parser("apply_C",
                                        help='If set, apply the calibration parameter to the input dataset to produce calibrated covariance matrices')
...
if args.subparser_name == 'measure':
    ...
    # Distortion matrix
    distortion_matrix = _np.diag(
        [1, f * g * _np.exp(1j * phi_t), f / g * _np.exp(1j * phi_r), f ** 2 * _np.exp(1j * (phi_t + phi_r))])
    # Invert it
