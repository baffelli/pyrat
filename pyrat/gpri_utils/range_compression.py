...
class gpriRangeProcessor:
    def __init__(self, args):
        ...
        self.rawdata = _np.fromfile(args.raw, dtype=self.raw_par.dt).reshape(
            [self.raw_par.block_length, self.raw_par.nl_tot][::-1]).T
        self.apply_scale = args.apply_scale
        self.rvp = _np.exp(1j * 4. * _np.pi * self.raw_par.grp.RF_chirp_rate * (
        self.raw_par.slr / C) ** 2)  # range video phase correction
    ...
    def compress(self, f_c=None, bw=66e6):
        ...
        arr_compr = _np.zeros((self.raw_par.ns_max - self.raw_par.ns_min + 1, self.raw_par.nl_tot_dec),
                              dtype=_np.complex64)
        ...
        for idx_az in range(0, self.raw_par.nl_tot_dec):
            ...
            if self.apply_scale:
                ...
                scale_factor = 1
            arr_compr[:, idx_az] = (
            line_comp[self.raw_par.ns_min:self.raw_par.ns_max + 1].conj() * scale_factor).astype('complex64')
    ...
    def fill_dict(self):
        ...
        #
        # Topsome receiver
        rx1_coord[0] = _gpf.xoff + _gpf.ant_radius * _np.cos(
        ...
        rx1_coord[2] = _gpf.rx1_dz[seq[1]] + _gpf.ant_radius * _np.sin(
