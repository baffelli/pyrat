...
def stft(x, fftsize=4096, overlap=50):
    ...
    fshift[1::2] = -1
    return _np.array([_np.fft.rfft(w[:, None] * x[i:i + fftsize, 1:], axis=0) * fshift[:, None] for i in
                      range(0, x.shape[0] - fftsize, hop)])
...
class rawTracker:
    ...
    def track_maximum(self):
        ...
        range_filter = _np.exp(
            1j * range_freq * _np.arange(0, self.grp.block_length - 1) / self.grp.grp.ADC_sample_rate)
        ...
        range_spectrum = range_spectrum[:,
                         max_az - self.args.analysis_window / 2:max_az + self.args.analysis_window / 2]
        # azimuth indices
        az_vec = (
                 max_az - self.args.analysis_window / 2) * self.grp.ang_per_tcycle + az_min + self.grp.ang_per_tcycle * _np.arange(
            range_spectrum.shape[1])
        # Window size for the filter
        freq_filter = _np.zeros(range_spectrum_shape[0])
        # Window position
        freq_filter[max_r - self.args.range_window / 2:max_r + self.args.range_window / 2] = _np.hamming(
            self.args.range_window)
...
def main():
    ...
    parser.add_argument('--search-window', dest='search_window', default=200, type=int,
                        help='Search window to look for the maxium response')
    parser.add_argument('--analysis-window', dest='analysis_window', default=50, type=int,
                        help='Azimuth Window for the anaylsis [samples]')
    parser.add_argument('--range-window', dest='range_window', default=100, type=int,
                        help='Range window for the spectral anaylsis [samples]')
