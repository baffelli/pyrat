...
def mcf(input, output, threads, config, params, wildcards):
    ...
    # unwrap data
    mcf_cmd = "mcf {input.ifgram} {input.cc} {input.cc_mask} {output.unw} {wd} {params.mode} - - - - 1 1 - {rinit} {rinit} 1 ".format(
        wd=wd, params=params, rinit=ref_coord[0], azinit=ref_coord[1])
    shell(mcf_cmd)
